...
use crate::fusion_engine_status_bar::FusionEngineStatusBar;
...
pub struct AdvancedInsTab<S: MessageSender> {
    client_sender: S,
    pub fusion_engine_status_bar: FusionEngineStatusBar<S>,
    imu_conf: u8,
    imu_temp: f64,
    rms_acc_x: f64,
    rms_acc_y: f64,
...
impl<S: MessageSender> AdvancedInsTab<S> {
    pub fn new(shared_state: SharedState, client_sender: S) -> AdvancedInsTab<S> {
        ...
        AdvancedInsTab {
            fusion_engine_status_bar: FusionEngineStatusBar::new(
                shared_state.clone(),
                client_sender.clone(),
            ),
            client_sender,
